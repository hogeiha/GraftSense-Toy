...
from drivers.dy_sv19t_driver import DYSV19T
from drivers.vibration_driver import VibrationSensor
# 导入任务模块
...
from tasks.sensor_task import musicTask
...
def vibration_handler()-> None:
    """
    振动传感器中断回调函数
    """
    global sensor_task_obj
    sensor_task_obj.count+= 1
...
time.sleep(3)
# 打印调试信息
print("FreakStudio : Vivration_music in GraftPort-RP2040")
...
# 创建震动传感器实例
vibration_pin = 14
vibration_sensor = VibrationSensor(pin=Pin(vibration_pin), callback=vibration_handler, debounce_ms=200)
vibration_sensor.init()

# 获取震动传感器引脚
dsv19t_tx = 12
dsv19t_rx = 13

# 初始化硬件串口：选择 UART1，波特率 9600
uart = UART(0, baudrate=9600, tx=Pin(dsv19t_tx), rx=Pin(dsv19t_rx))
...
# 创建传感器-蜂鸣器-LED任务实例
sensor_task_obj = musicTask(VibrationSensor=vibration_sensor, DYSV19T=player, debug=True)
